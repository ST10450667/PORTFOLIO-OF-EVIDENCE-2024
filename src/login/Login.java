...
public class Login {
    String Name; //Holds username when user logs in
    String LinkPassword; //Holds password when user logs in
     String FirstName ; //Holds firstname when user registers
     String LastName ; //Holds lastname when user registers
   String Insertpassword ; //Holds the password entered when user registers
     String InsertuserName; //Holds username entered when user registers
   ...
   boolean check=false;
    for(int i=0; i<InsertuserName.length(); i++){
   if(InsertuserName.length() <=5){
       
   if((int)InsertuserName.charAt(i) ==95)
       check=true;
   }
    }
    return check;
 ...
 for(int i=0; i<Insertpassword.length(); i++){
   if(Insertpassword.length() >= 8){ 
   if((int)Insertpassword.charAt(i) >65 &&(int)Insertpassword.charAt(i) <=90) {
       CapitalLetter=true;
   }  
  if((int)Insertpassword.charAt(i) >=48 &&(int)Insertpassword.charAt(i) <=57){
       Number=true;
               }          
   if((int)Insertpassword.charAt(i) >=33 &&(int)Insertpassword.charAt(i) <=47 || 
           (int)Insertpassword.charAt(i) >=58 &&(int)Insertpassword.charAt(i) <=64 || 
           (int)Insertpassword.charAt(i) >=91 &&(int)Insertpassword.charAt(i) <=96 || 
           (int)Insertpassword.charAt(i) >=123 &&(int)Insertpassword.charAt(i) <=126)   
          Special=true;
   }
...
return CapitalLetter && Number && Special;
 ...
 if(InsertuserName.equals(Name) && (Insertpassword.equals(LinkPassword))){
...
System.out.println("Enter Lastname :");
           login.LastName=input.next();
            System.out.println("Enter Username :");
                    login.InsertuserName=input.next();
           System.out.println("Enter Password :");
                    login.Insertpassword=input.next();

             System.out.println(login.registerUser());   
          if(login.checkUsername()==true & (login.checkPasswordComplexity())==true){

              System.out.println("Enter Username :");
     login.Name=input.next();
    System.out.println("Enter Password :");
       login.LinkPassword=input.next();
